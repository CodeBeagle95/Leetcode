...
public: 
    ...
    bool isScramble(string s1, string s2) {
        ...
        vector<vector<vector<bool>>> dp(s1.size(), 
            vector<vector<bool>>(s1.size(), 
                vector<bool>(s1.size(), false)
            )
        );

        for (int l = 1; l <= s1.size(); ++l) { // 长度 l 要放在最外一层循环
            for (int i = 0; i + l <= s1.size(); ++i) {
                for (int j = 0; j + l <= s1.size(); ++j) {
                    if (l == 1) {
                        dp[i][j][0] = (s1[i] == s2[j]); // 底层赋值
                    }
                    for (int k = 1; k < l; ++k) { // l 为长度, 分解为 k, l - k
                        if (dp[i][j][k - 1] && dp[i + k][j + k][l - k - 1] ||     // 正序匹配
                            dp[i][j + l - k][k - 1] && dp[i + k][j][l - k - 1]) { // 逆序匹配
                            dp[i][j][l - 1] = true;
                            break;
                        }
                    }
